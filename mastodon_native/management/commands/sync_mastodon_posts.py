"""
Management command to sync Mastodon posts that were published outside of PostFlow.

This command fetches all posts from connected Mastodon accounts and creates
ScheduledPost records for any posts not already in the system. This allows users to
view analytics for all their Mastodon content in one place.

Usage:
    python manage.py sync_mastodon_posts
    python manage.py sync_mastodon_posts --account-id 123
    python manage.py sync_mastodon_posts --limit 40
"""
import logging
import requests
...
class Command(BaseCommand):
    help = 'Sync Mastodon posts from connected accounts into PostFlow'

    def add_arguments(self, parser):
        parser.add_argument(
            '--account-id',
            type=int,
            help='Sync posts from a specific Mastodon account ID'
        )
        parser.add_argument(
            '--limit',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if account_id:
            ...
            if not accounts.exists():
                self.stdout.write(
                    self.style.ERROR(f'Mastodon account with ID {account_id} not found')
                )
                return
        else:
        ...
        self.stdout.write(f'Found {total_accounts} Mastodon account(s) to sync\n')
        ...
        for account in accounts:
            ...
            try:
                ...
                logger.exception(f"Error syncing Mastodon account {account.id}")
                total_errors += 1
    ...
    def sync_account_posts(self, account, limit: int, force: bool):
        """
        Sync posts from a single Mastodon account.
        ...
        try:
            ...
            # Fetch posts (statuses) from Mastodon
            ...
            self.stdout.write(f'  Found {len(statuses)} posts on Mastodon')

            for status in statuses:
                try:
                    # Check if we already have this post
                    mastodon_post_id = status.get('id')

                    if not force:
                        existing_post = ScheduledPost.objects.filter(
                            mastodon_post_id=mastodon_post_id
                        ).filter(
                            mastodon_native_accounts=account
                        ).first()
                    ...
                    # Get caption/content
                    # Mastodon returns HTML content, we need to strip it
                    ...
                    with transaction.atomic():
                        ...
                        synced += 1
                        self.stdout.write(
                            f'  ✓ Synced post {mastodon_post_id} from {post_date.strftime("%Y-%m-%d")}'
                        )
                    ...
                    logger.error(f"Error processing Mastodon post: {e}")

        except requests.exceptions.RequestException as e:
            self.stdout.write(
                self.style.ERROR(f'  ✗ API error: {e}')
            )
            logger.error(f"Mastodon API error for account {account.id}: {e}")
            errors += 1
